...
def generate_sql(text):
    ...
    try:
        mapper, xml_raw_text = mybatis_mapper2sql.create_mapper(
            xml_raw_text=text)
        statements = mybatis_mapper2sql.get_statement(
...
def _extract_tables_by_moz(moz_parser_dict, tables=None, parent_keyword=None, is_alias=False):
    ...
    if isinstance(moz_parser_dict, dict):
        for k in moz_parser_dict.keys():
            ...
            _extract_tables_by_moz(
                moz_parser_dict[k], tables, parent_keyword, is_alias)
...
def check_select_statement(statement, rgx='select'):
    if isinstance(statement, str) and statement:
        token = sqlparse.parse(statement)[0].tokens[0]
        if token.is_keyword and token.value == rgx:
            return True
    return False
