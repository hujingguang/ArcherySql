...
from sql.utils.sql_utils import check_select_statement
...
class InceptionEngine(EngineBase):
    def get_connection(self, db_name=None):
        ...
        if hasattr(self, 'instance'):
            self.conn = MySQLdb.connect(
                host=self.host, port=self.port, charset=self.instance.charset or 'utf8mb4')
            return self.conn
        archer_config = SysConfig()
        inception_host = archer_config.get('inception_host')
        inception_port = int(archer_config.get('inception_port', 6669))
        self.conn = MySQLdb.connect(host=inception_host,
                                    port=inception_port,
                                    charset='utf8mb4')
        return self.conn
    ...
    def get_backup_connection():
        ...
        backup_port = int(archer_config.get(
    ...
    def execute_select_check(self, instance=None, db_name=None,
                             sql='', conn=None):
        """inception check"""
        check_result = ReviewSet(full_sql=sql)
        # 检查 inception 不支持的函数
        ...
        for statement in sqlparse.split(sql):
            ...
            if re.match(r"(\s*)alter(\s+)table(\s+)(\S+)(\s*);|(\s*)alter(\s+)table(\s+)(\S+)\.(\S+)(\s*);",
                ...
                result = ReviewResult(id=line, errlevel=0,
                                      sql=statement, stagestatus="Audit Completed", errormessage=None)
            check_result.rows += [result]
            line += 1
        if check_result.is_critical:
            return check_result
        ...
        for statement in sqlparse.split(sql):
            statement = sqlparse.format(statement, strip_comments=True)
            if check_select_statement(statement, rgx="use"):
                continue
            elif check_select_statement(statement):
                do_sql = statement
        ...
        test_sql = do_sql.replace(
            ";", "") + " limit 1;" if "limit" not in do_sql else do_sql
        check_result.rows = []
        if conn:
            try:
                cursor = conn.cursor()
                ...
                do_sql = do_sql.replace(
                    ";", "") + " limit 10000;" if "limit" not in do_sql else do_sql
                result = ReviewResult(id=1, errlevel=0, sql=do_sql)
            ...
            result = ReviewResult(id=1,
                                  ...
                                  errormessage='无法检测语句',
                                  sql=do_sql)
            check_result.error = "无法检测语法"
            check_result.error_count += 1
        check_result.rows = [result]
        check_result.syntax_type = 0  # TODO 工单类型 0、其他 1、DDL，2、DML 仅适用于MySQL，待调整
        check_result.checked = True
        return check_result
    ...
    def execute(self, workflow=None):
        ...
        for splitRow in split_result.rows:
            ...
            sql_execute = f"""/*--user={instance.user};--password={instance.raw_password};--host={instance.host};
                                ...
                                {sql_tmp}
                                inception_magic_commit;"""
            one_line_execute_result = self.query(
                sql=sql_execute, close_conn=False)
            # 把结果转换为ReviewSet
