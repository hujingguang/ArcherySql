...
def sql_workflow_list(request):
    ...
    # 返回查询结果
    result = json.dumps(result, cls=ExtendJSONEncoder, bigint_as_string=True)
...
def select_check(request):
    ...
    try:
        check_engine = get_engine(instance=instance)
        check_result = check_engine.execute_select_check(
            db_name=db_name, sql=sql_content.strip())
    except Exception as e:
        result['status'] = 1
        result['msg'] = str(e)
...
def submit(request):
    ...
    list_cc_addr = [email['email'] for email in Users.objects.filter(
    ...
    auto_review_wrong = sys_config.get(
        'auto_review_wrong', '')  # 1表示出现警告就驳回，2和空表示出现错误才驳回
    ...
    try:
        ...
        if workflow_status == 'workflow_manreviewing':
            ...
            async_task(notify_for_audit, audit_id=audit_id,
                       email_cc=list_cc_addr, timeout=60)

    return HttpResponseRedirect(reverse('sql:detail', args=(workflow_id,)))


@permission_required('sql.sql_submit', raise_exception=True)
def submit_select(request):
    """正式提交SQL, 此处生成工单"""
    sql_content = request.POST.get('sql_content').strip()
    workflow_title = request.POST.get('workflow_name')
    # 检查用户是否有权限涉及到资源组等， 比较复杂， 可以把检查权限改成一个独立的方法
...
def execute(request):
    ...
    # 将流程状态修改为对应状态
    SqlWorkflow(id=workflow_id, status=status, finish_time=finish_time).save(
...
def cancel(request):
    ...
    try:
        ...
        if audit_detail.current_status == WorkflowDict.workflow_status['audit_abort']:
            async_task(notify_for_audit, audit_id=audit_detail.audit_id,
                       audit_remark=audit_remark, timeout=60)
    return HttpResponseRedirect(reverse('sql:detail', args=(workflow_id,)))
