...
def sqldataout(request):
    ...
    return render(request, 'sqldataout.html',
                  {'status_list': SQL_WORKFLOW_CHOICES,
                   'instance': instance, 'resource_group': resource_group})
...
def submit_sql(request):
    ...
    # 主动创建标签
    InstanceTag.objects.get_or_create(tag_code='can_write', defaults={
                                      'tag_name': '支持上线', 'active': True})
...
def submit_select_sql(request):
    ...
    context = {'active_user': active_user, 'group_list': group_list,
               'enable_backup_switch': archer_config.get('enable_backup_switch')}
    return render(request, 'sqlSelectsubmit.html', context)
...
def detail(request, workflow_id):
    ...
    context = {'workflow_detail': workflow_detail, 'rows': rows, 'last_operation_info': last_operation_info,
               ...
               'run_date': run_date, 'syntax_type': workflow_detail.syntax_type}
...
def rollback(request):
    ...
    rollback_workflow_name = "【回滚工单】原工单Id:%s ,%s" % (
...
def sqlquery(request):
    """SQL在线查询页面"""
    # 主动创建标签
    InstanceTag.objects.get_or_create(tag_code='can_read', defaults={
                                      'tag_name': '支持查询', 'active': True})
    return render(request, 'sqlquery.html')


@permission_required('sql.menu_output', raise_exception=True)
def sqloutput(request):
    """SQL在线导出页面"""
    # 主动创建标签
    InstanceTag.objects.get_or_create(tag_code='can_read',
                                      defaults={'tag_name': '支持导出', 'active': True})
    return render(request, 'sqloutput.html')
    ...
    if workflow_detail.status == 2:
        try:
            audit_id = Audit.detail_by_workflow_id(
                workflow_id=apply_id, workflow_type=1).audit_id
            last_operation_info = Audit.logs(
                audit_id=audit_id).latest('id').operation_info
        except Exception as e:
            logger.debug(f'无审核日志记录，错误信息{e}')
...
def queryuserprivileges(request):
    """查询权限管理页面"""
    # 获取所有用户
    user_list = QueryPrivileges.objects.filter(
