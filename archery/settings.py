...
CACHES = {
    "default": {
        ...
        "LOCATION": "redis://10.210.20.47:6379/0",
        "OPTIONS": {
...
if ENABLE_LDAP:
    ...
    AUTHENTICATION_BACKENDS = (
        ...
        # django系统中手动创建的用户也可使用，优先级靠后。注意这2行的顺序
        'django.contrib.auth.backends.ModelBackend',
    )

    BASE_DN = "DC=it,DC=com"
    AUTH_LDAP_SERVER_URI = "ldap://x.x.x.x"
    AUTH_LDAP_BIND_DN = 'CN=深圳运维监控中心,OU=各系统LDAP认证账户,DC=it,DC=com'
    AUTH_LDAP_BIND_PASSWORD = 'xxxx'
    AUTH_LDAP_GROUP_SEARCH = LDAPSearch("CN=深圳运维监控中心,OU=各系统LDAP认证账户,DC=ucredit,DC=com",
                                        ldap.SCOPE_SUBTREE, "(objectClass=groupOfNames)"
                                        )

    AUTH_LDAP_GROUP_TYPE = GroupOfNamesType()
    AUTH_LDAP_USER_FLAGS_BY_GROUP = {
        "is_staff": 'CN=深圳运维监控中心,OU=各系统LDAP认证账户,DC=it,DC=com',
    }
    AUTH_LDAP_USER_SEARCH = LDAPSearch(
        BASE_DN, ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))")
